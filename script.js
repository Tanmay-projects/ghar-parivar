...
async function loadPrivate(){const box=$("private-grid");if(!box)return;try{if(!state.user)throw new Error("Not logged in");$("welcome-user").textContent=`स्वागत है, ${state.user.username}`;state.privateData=await api("/family/private");const members=state.privateData.members||[];box.innerHTML=members.map(m=>`<article class="private-card"><h3>${m.name}</h3><p class="private-designation">${m.designation||"परिवार सदस्य"}</p></article>`).join("")||"<p>अभी निजी सदस्य जानकारी उपलब्ध नहीं है।</p>"}catch(err){box.innerHTML=`<p>${err.message}</p>`}}
